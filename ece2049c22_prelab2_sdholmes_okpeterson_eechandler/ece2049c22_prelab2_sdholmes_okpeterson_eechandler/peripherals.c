...
/*
 * Function to configure the 4 lab board buttons
 */
void configUserBtns(void)
{
    P7SEL &= ~(BIT0 | BIT4); //Select function for S1 and S4
    P3SEL &= ~BIT6;       //Select function for S2
    P2SEL &= ~BIT2;       //Select Function for S3
...
void initLeds(void)
{
    ...
    P6SEL &= ~(BIT4|BIT3|BIT2|BIT1);    //select them as digital i/o
    P6DIR |=  (BIT4|BIT3|BIT2|BIT1);    //output pins
    P6OUT &= ~(BIT4|BIT3|BIT2|BIT1);    //clear output
}
...
void BuzzerOn(int num)
{
    ...
    period = num;
    TB0CCR0 = period/5; // Set the PWM period in ACLK ticks
...
void configKeypad(void)
{
    ...
    P2DIR |= (BIT5|BIT4);
    P1DIR |= BIT5;
    P2OUT |= (BIT5|BIT4); //set default to 1
    P1OUT |= BIT5;        //

    // Rows are input
    P1DIR &= ~(BIT2|BIT3|BIT4);
    P4DIR &= ~(BIT3);
    P4REN |= (BIT3);  //pull up or down
